...
void MobIPv6mn::handleMihMessage(cMessage *msg)
{
	...
	IPv6Packet * locUpdate = new IPv6Packet("BindingUpdate");
...
{
	IPv6Packet * ip = check_and_cast<IPv6Packet*>(msg);
...
void MobIPv6mn::handleMessage(cMessage *msg)
{
	...
	if (!pkt) {
		IPv6Packet *ip = dynamic_cast<IPv6Packet*>(msg);
	...
	IPv6Packet * ip = new IPv6Packet(tmp.str().c_str());
	//ip->encapsulate(msg);
