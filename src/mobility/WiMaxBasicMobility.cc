...
#include "WiMaxBasicMobility.h"
#include "FWMath.h"
#include <string.h>

#define coreEV (ev.isDisabled()||!coreDebug) ? (std::ostream&)ev : EV << logName() << "::WiMaxBasicMobility: "
 ...
 * INET FIX...
 *
 * In order to allow two different wireless channels we changed the basic mobility
 * NOT TO LOOK FOR "channelControl" but for ANYTHING defined in the parameter
 * controlModuleName.
 *
 * This way we can have 2 different channels for WiMax and for WiFi
 */
void WiMaxBasicMobility::initialize(int stage)
{
    BasicModule::initialize(stage);

    EV << "initializing WiMaxBasicMobility stage " << stage << endl;

    if (stage == 0)
    {
    	std::string mod_name = par("controlModuleName").stdstringValue();
    	cc = dynamic_cast<ChannelControl *>(simulation.getModuleByPath(mod_name.c_str()));

    	std::string lower = mod_name;
    	for (uint i = 0; i<lower.length(); i++)
    		lower[i]=tolower(mod_name[i]);


    	if (!cc)
    		cc = dynamic_cast<ChannelControl *>(simulation.getModuleByPath(lower.c_str()));

    	if (cc) EV<<cc->getName()<<endl;

		if (!cc)
			throw cRuntimeError("Could not find %s module", mod_name.c_str());
    ...
    else if (stage == 1)
    {
        ...
        // read the playground size from ChannelControl
        Coord pgs = cc->getPgs();
...
void WiMaxBasicMobility::updatePosition()
{
 ...
 * calculation
 */
Coord WiMaxBasicMobility::getRandomPosition()
{
    Coord p;
...
void WiMaxBasicMobility::reflectIfOutside(Coord& targetPos, Coord& step, double& angle)
{
    if (pos.x < 0)
    {
        pos.x = -pos.x;
...
void WiMaxBasicMobility::wrapIfOutside(Coord& targetPos)
{
    if (pos.x < 0)
    {
...
void WiMaxBasicMobility::placeRandomlyIfOutside(Coord& targetPos)
{
...
void WiMaxBasicMobility::raiseErrorIfOutside()
{
    if (pos.x<0 || pos.x>=getPlaygroundSizeX() || pos.y<0 || pos.y>=getPlaygroundSizeY())
    {
...
void WiMaxBasicMobility::handleIfOutside(BorderPolicy policy, Coord& targetPos, Coord& step, double& angle)
{
    switch (policy)
    {
