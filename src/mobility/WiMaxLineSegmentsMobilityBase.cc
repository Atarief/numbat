...
void WiMaxLineSegmentsMobilityBase::initialize(int stage)
{
    WiMaxBasicMobility::initialize(stage);
...
void WiMaxLineSegmentsMobilityBase::beginNextMove(cMessage *msg)
{
    ...
    if (targetTime<now)
        error("WiMaxLineSegmentsMobilityBase: targetTime<now was set in %s's beginNextMove()", getClassName());
...
void WiMaxLineSegmentsMobilityBase::handleSelfMsg(cMessage *msg)
{
    if (stationary)
    {
        delete msg;
