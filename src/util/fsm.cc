...
void Fsm::onEvent(FsmEventType e, cMessage *msg)
{
	...
	opp_error("%s: Invalid state (%d) returned during %s event processing in state %s.", getFullName(),
...
void Fsm::stateSet(FsmStateType newState)
{
	...
	if (transitionsCnt > FSM_MAX_TRANSITIONS)
	    opp_error("%s: probably state transitions loop detected: %d transitions occurred without stationary state.\n",
